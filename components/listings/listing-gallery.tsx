...
export function ListingGallery({ images }: { images: Image[] }) {
  ...
  if (images.length === 0) {
    return (
      <div className="mt-6 flex aspect-[16/9] items-center justify-center rounded-3xl bg-stone-100 text-stone-400">
        No images
      </div>
    );
  }
    ...
    <div className="mt-6">
      <div className="grid gap-3 sm:grid-cols-[2fr_1fr]">
        <button
          ...
          className="group relative aspect-[5/4] overflow-hidden rounded-3xl bg-stone-100"
        >
          ...
          />
          <span className="absolute right-4 top-4 inline-flex items-center gap-1.5 rounded-full bg-white/90 px-3 py-1.5 text-xs font-medium text-stone-800 shadow-sm">
          ...
          {rest.length > 0
            ? rest.map((img, i) => (
                <button
                  ...
                  className="relative aspect-[5/4] overflow-hidden rounded-2xl bg-stone-100"
                >
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img
                    src={img.url}
            ...
            : Array.from({ length: 2 }, (_, i) => (
                <div
                  key={i}
                  className="aspect-[5/4] rounded-2xl bg-stone-100"
                />
              ))}
        </div>
      </div>
      ...
      {lightbox && (
        <div className="fixed inset-0 z-50 grid place-items-center bg-stone-950/90 p-4 animate-fade-in">
          <button
            type="button"
